...
from models.nnUnet.neural_network import SegmentationNetwork
...
class Generic_UNet(SegmentationNetwork):
    ...
    def forward(self, x):
        ...
        def crop_to_match(tensor, target):
            ...
            # 假设输入的 tensor 和 target 维度是 [batch_size, channels, height, width]
            diff_y = tensor_shape[2] - target_shape[2]
            diff_x = tensor_shape[3] - target_shape[3]

            # 只裁剪多余的部分
            if diff_y > 0:
                tensor = tensor[:, :, :-diff_y, :]
            if diff_x > 0:
                tensor = tensor[:, :, :, :-diff_x]

            return tensor

        for u in range(len(self.tu)):
            x = self.tu[u](x)
            # x = crop_to_match(x, skips[-(u + 1)])  # 裁剪上采样特征图尺寸以匹配跳跃连接
...
def initialize_network(threeD=True, num_classes=2):
    ...
    if threeD:
        ...
        pool_op_kernel_sizes = default_dict["net_num_pool_op_kernel_sizes"]  # 3D配置
        conv_kernel_sizes = default_dict["net_conv_kernel_sizes"]  # 3D配置
    else:
        conv_op = nn.Conv2d
        dropout_op = nn.Dropout2d
        norm_op = nn.InstanceNorm2d
        # 修改为二维池化步长和卷积核大小
        pool_op_kernel_sizes = [(2, 2)] * len(default_dict["net_num_pool_op_kernel_sizes"])
        conv_kernel_sizes = [(3, 3)] * (len(default_dict["net_num_pool_op_kernel_sizes"]) + 1)
    ...
    network = Generic_UNet(default_dict["num_input_channels"], default_dict["base_num_features"],
                           ...
                           len(pool_op_kernel_sizes), default_dict["conv_per_stage"], 2, conv_op, norm_op,
                           norm_op_kwargs,
                           dropout_op, dropout_op_kwargs, net_nonlin, net_nonlin_kwargs, False, False,
                           lambda x: x, InitWeights_He(1e-2), pool_op_kernel_sizes, conv_kernel_sizes,
                           False, True, True)
    print("nnUNet have {} paramerters in total".format(
        sum(x.numel() for x in network.parameters())))
    return network

# input = torch.FloatTensor(1, 1, 224, 224)
# input_var = input.cuda()
# model = initialize_network(threeD=False).cuda()
# out = model(input_var)
# print(out.size())