...
def MSFR(in_channels, out_channels, n=1, stride=1, kernel_sizes=[1, 3, 5], expansion_factor=2, dw_parallel=True,
              add=True, activation='relu6'):
    """
...
#         self.dwconvs = nn.ModuleList([
#             nn.Sequential(
#                 nn.Conv2d(self.in_channels, self.out_channels, kernel_size, stride, kernel_size // 2,
#                           groups=1, bias=False),
#                 nn.BatchNorm2d(self.out_channels),
#                 act_layer(self.activation, inplace=True)
#             )
...
#         self.mscb4 = MSFR(channels[0], channels[0], n=1, stride=1, kernel_sizes=kernel_sizes,
...
#         self.mscb3 = MSFR(channels[1], channels[1], n=1, stride=1, kernel_sizes=kernel_sizes,
#                                expansion_factor=expansion_factor, dw_parallel=dw_parallel, add=add,
#                                activation=activation)
#
...
#         self.msfr2 = MSFR(channels[2], channels[2], n=1, stride=1, kernel_sizes=kernel_sizes,
#                                expansion_factor=expansion_factor, dw_parallel=dw_parallel, add=add,
#                                activation=activation)
#
...
#         self.msfr1 = MSFR(channels[3], channels[3], n=1, stride=1, kernel_sizes=kernel_sizes,
#                                expansion_factor=expansion_factor, dw_parallel=dw_parallel, add=add,
#                                activation=activation)
#
...
class MSFRAM(nn.Module):
    def __init__(self, in_channels, out_channels, n=1, stride=1, kernel_sizes=[1, 3, 5], expansion_factor=2,
                 dw_parallel=True,
                 add=True, activation='relu6'):
        super(MSFRAM, self).__init__()

        self.msfr1 = MSFR(in_channels, out_channels)
        #self.mymsdc = MYMSDC(in_channels,out_channels,kernel_sizes=kernel_sizes,stride=stride)
        self.msfr2 = MSFR(out_channels, out_channels)
